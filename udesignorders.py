...
q = """SELECT user.username,user.door,user.street,user.city,user.district,user.state,user.pincode,user.phno,userdesign.quantity,userdesign.size,userdesign.price,userdesign.totalprice,userdesign.status,userdesign.id,userdesign.referenceimage,userdesign.dresstype,userdesign.userid FROM userdesign INNER JOIN user ON userdesign.userid = user.id where userdesign.userid="%s" AND userdesign.status!="NULL" AND userdesign.accept!="NULL" """ % ID
cur.execute(q)
rec = cur.fetchall()
...
for i in rec:
    ...
    """%(i[10],i[11],i[13],i[15],i[15],i[8],i[9],i[10],i[11],i[16],i[13]))
    if i[12]=="Discard":
        print("""
...
if placeorder != None:
    if len(form) != 0:
        q = """Update userdesign set accept="%s" where id="%s" """ % (placeorder, oid)
        cur.execute(q)
        q11 = """INSERT INTO orders(productid, productname, category, quantity, size, price, totalprice, card, cardno, expirydate, userid)VALUES('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')""" % (proid,proname,category, quan, size, price,prize, card, cardno, exdate,uid)
        cur.execute(q11)
        con.commit()
        print("""
                    <script>
                ...
                """)
    if cancel != None:
        q1="""Update userdesign set accept="%s" where id="%s" """ % (cancel, oid)
        cur.execute(q1)
        print("""
                            <script>
