...
<body>
    <div class="wrapper">
        ...
        <div class="tabular-wrapper" >
            <h3 class="min-title" style="font-size: 29px;margin-left:100px;"><b>New Product Details</b></h3>
...
for i in rec:
    fn1 = "product/" + i[3]
    if i[12] != 'Approved' and i[12] != 'Discard':
        print("""
                                <tr>
                                            <td>%s</td>
                                            <td>%s</td>
                                ...
                                """%(i[0],i[1],i[2],fn1,i[4],i[5],i[6],i[7],i[8],i[9],i[0],i[1]))

form = cgi.FieldStorage()
id=form.getvalue("id")
productid=form.getvalue("proid")
approve = form.getvalue("approve")
discard = form.getvalue("discard")
if approve != None:
        x = """Update product set status = "%s" where id="%s" """ %(approve,id) # Assuming 1 is a valid ID for testing

        # x = """Update product set status = "%s" where productid="%s" """ %(approve,productid)
        ...
        print("""
                ...
                """)
if discard != None:
        x = """Update product set status = "%s" where id="%s" """ %(discard,id)
        cur.execute(x)
        con.commit()
        print("""
        <script>
