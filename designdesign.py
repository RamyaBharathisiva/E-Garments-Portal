...
q = """SELECT user.username,user.door,user.street,user.city,user.district,user.state,user.pincode,user.phno,userdesign.quantity,userdesign.size,userdesign.price,userdesign.totalprice,userdesign.status,userdesign.id,userdesign.referenceimage,userdesign.dresstype FROM userdesign INNER JOIN user ON userdesign.userid = user.id where userdesign.userid="%s" AND userdesign.status!="NULL" AND receive!="Product Received" AND action = "Product Delivered" """ % ID
cur.execute(q)
rec = cur.fetchall()
