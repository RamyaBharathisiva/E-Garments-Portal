...
for i in rec1:
    ...
    print(""" <div id="myModal-%s" class="modal fade" role="dialog">
                    <div class="modal-dialog modal-lg">
                        <form action="" name="order" id="login" method="post" enctype="multipart/form-data">
...
if order != None:
    if len(form) != 0:
        q = """insert into orders(productid,productname,category,quantity,size,price,totalprice,card,cardno,expirydate,userid)VALUES('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')""" % (proid,proname,category, quan, size, price,prize, card, cardno, exdate,TID)
        cur.execute(q)
        con.commit()
        print("""
                    <script>
