...
cur = con.cursor()
q = """Select * from product order by id DESC limit 5 """
cur.execute(q)
rec1 = cur.fetchall()
...
for i in rec1:
    fn = "product/" + i[3]
    print("""<div class="col-md-3" style="margin-top:20px;" height="300px" width="300px">
...
if order != None:
    if len(form) != 0:
        q = """insert into orders(productid,productname,category,quantity,size,price,card,cardno,expirydate)VALUES('%s','%s','%s','%s','%s','%s','%s','%s','%s')""" % (
            proname, proid, category, quan, size, prize, card, cardno, exdate)
        cur.execute(q)
        con.commit()
        print("""
                    <script>
