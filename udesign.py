...
if submit != None:
    if len(form) != 0:
        ...
        if Referanceimage.filename:
            ...
            q="""insert into userdesign(userid,designfor,gender,dresstype,fabric,color,size,age,referenceimage,quantity,additionalrequest)values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"""%(ID,design,gender,dresstype,frabric,color,size,age,fn,productquantity,request)
            cur.execute(q)
            con.commit()
            print("""
                     <script>
