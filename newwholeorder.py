...
cur = con.cursor()
q = """SELECT wholeorders.id,wholesaler.ownername,wholesaler.shopname,wholesaler.door,wholesaler.street,wholesaler.city,wholesaler.district,wholesaler.state,wholesaler.pincode,wholesaler.phno,wholeorders.productname,wholeorders.quantity,wholeorders.size,wholeorders.price,wholeorders.totalprice,wholeorders.card,wholeorders.cardno,wholeorders.expirydate,wholeorders.status,wholeorders.category,wholeorders.ordertime FROM wholeorders INNER JOIN wholesaler ON wholeorders.userid = wholesaler.id """
cur.execute(q)
rec = cur.fetchall()
print("""
<!DOCTYPE html>
...
<div class="main--container">
            <div class="main--title">
                <div class="col-md-10">
                    <h1><b>Wholesaler New Orders</b></h1>
                </div>
            </div>
...
""")
for i in rec:
    if i[18] != "Confirm" and i[18] != "Cancel":
        print("""
    ...
    """ % (i[0], i[2],i[10], i[19], i[11], i[12], i[13], i[14], i[15], i[16], i[17], i[0], i[9], i[3], i[4], i[5], i[6], i[7],
           i[8], i[20], i[0]))
...
if orderid != None:
    if confirm != None:
        q = """Update wholeorders set status = "%s" where id="%s" """ % (confirm, orderid)
        cur.execute(q)
        con.commit()
        print("""
        <script>
    ...
    if delete != None:
        q = """Update wholeorders set status = "%s" where id="%s" """ % (delete, orderid)
        cur.execute(q)
        con.commit()
        print("""
            <script>
